...
int main()
{
	...
	// NOTE(rswinkle): This is really inefficient interpolating texcords we don't use but I'm doing
	// it for the same reason I don't combine model,view,projection client side, to
	// have a more direct port...maybe I'll go through all these later making small
	// obvious optimizations
	GLuint shaderSingleColor = pglCreateProgram(stencil_vs, stencil_single_color_fs, 2, smooth, GL_FALSE);
	...
	while (true)
	{
		...
		// set uniforms
		// glUseProgram(shaderSingleColor); // No need since both shaders use the same uniform pointer
		...
		uniforms.model = glm::scale(model, glm::vec3(scale, scale, scale));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		model = mat4(1.0f);
...
void stencil_single_color_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	*(vec4*)&builtins->gl_FragColor = vec4(0.04, 0.28, 0.26, 1.0);
}
