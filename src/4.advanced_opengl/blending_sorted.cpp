...
int main()
{
	...
	while (true)
	{
		...
		glBindVertexArray(transparentVAO);
		uniforms.tex = transparentTexture;
		// Could have used auto here, the one type of situation where I believe
		// it actually should be used, but for now matching his code
		for (std::map<float, vec3>::reverse_iterator it = sorted.rbegin(); it != sorted.rend(); ++it)
		{
			model = mat4(1.0f);
			uniforms.model = translate(model, it->second);
			glDrawArrays(GL_TRIANGLES, 0, 6);
		}
...
void blending_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	...
	builtins->gl_FragColor = texColor;
}
...
// ---------------------------------------------------
unsigned int loadTexture(char const * path, GLenum wrapmode)
{
