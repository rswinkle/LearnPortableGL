...
int main()
{
	...
	// -----------------------------------------------------------
	GLenum smooth[] = { SMOOTH, SMOOTH, SMOOTH, SMOOTH, SMOOTH, SMOOTH, SMOOTH, SMOOTH, SMOOTH };
	GLuint lightingShader = pglCreateProgram(basic_lighting_vs, basic_lighting_fs, 9, smooth, GL_FALSE);
...
void basic_lighting_vs(float* vs_output, void* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	*(vec4*)&builtins->gl_Position = projection * view * model * aPos;
	...
	// FragPos = 
	*(vec3*)vs_output = vec3(view * model * aPos);

	// Normal = 
	((vec3*)vs_output)[1] = mat3(transpose(inverse(view * model))) * aNormal;

	// LightPos = 
	((vec3*)vs_output)[2] = vec3(view * vec4(lightPos, 1.0f));
}
...
void basic_lighting_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	...
	vec3 viewDir = normalize(-FragPos); // viewer is always at (0,0,0) in view-space, so viewDir is (0,0,0) - Position => -Position
