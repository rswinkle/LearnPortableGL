...
int main()
{
	...
	GLuint lightingShader = pglCreateProgram(basic_lighting_vs, basic_lighting_fs, 3, smooth, GL_FALSE);
...
void basic_lighting_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	*(vec4*)&builtins->gl_Position = projection * view * model * aPos;
	...
	vec3 lightDir = normalize(u->lightPos - Position);
	...
	vec3 viewDir = normalize(u->viewPos - Position);
	vec3 reflectDir = reflect(-lightDir, norm);
	float spec = pow(max(dot(viewDir, reflectDir), 0.0f), 32);
	vec3 specular = specularStrength * spec * u->lightColor;

	// out vec3 LightingColor
	*(vec3*)vs_output = ambient + diffuse + specular;
}
...
void basic_lighting_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	...
	vec3 LightingColor = ((vec3*)fs_input)[0];

	*(vec4*)&builtins->gl_FragColor = vec4(LightingColor * u->objectColor, 1.0f);
}
