...
int main()
{
	...
	while (true)
	{
		...
		glUseProgram(lightingShader);
		uniforms.light.position = camera.Position;
		uniforms.light.direction = camera.Front;
		uniforms.light.cutOff = cos(radians(12.5f));
		uniforms.viewPos = camera.Position;

		// light properties
        // we configure the diffuse intensity slightly higher; the right lighting conditions differ with each lighting method and environment.
        // each environment and lighting type requires some tweaking to get the best out of your environment.
		uniforms.light.ambient = vec3(0.1f);
		uniforms.light.diffuse = vec3(0.8f);
...
void materials_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	...
	if (theta > light.cutOff) {
		...
		// remove attenuation for ambient, otherwise at large distances
		// the light would be darker inside than outside the spotlight
		//ambient  *= attenuation;
		diffuse   *= attenuation;
		...
		*(vec4*)&builtins->gl_FragColor = vec4(result, 1.0f);
		...
		// else use ambient light so scene isn't completely dark outside spotlight
		*(vec4*)&builtins->gl_FragColor = vec4(ambient, 1.0f);
	}
}
