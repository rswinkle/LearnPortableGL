...
int main()
{
	...
	GLuint shader = pglCreateProgram(shader_vs, shader_fs, 14, smooth, GL_FALSE);
	glUseProgram(shader);
	...
	while (true)
	{
		...
		model = rotate(model, radians((SDL_GetTicks() / 1000.0f) * -10.0f), normalize(vec3(1.0, 0.0, 1.0))); // rotate the quad to show normal mapping from multiple directions
