...
int main()
{
	// Guest CSM article uses a geometry shader + sampler2DArray (5 cascades at 4096).
	// PGL has neither GS nor texture arrays; this is the ch5 directional shadow map
	// as a runnable stand-in. See scratch/ch8_ai_port_notes.md.
...
static float ShadowCalculation(vec4 fragPosLightSpace, vec3 normal, vec3 fragPos, vec3 lightPos, GLuint shadowMap)
{
	...
	float shadow = 0.0f;
	pgl_ivec3 shadowMapSize = textureSize(shadowMap, 0);
	vec2 texelSize = 1.0f / vec2(shadowMapSize.x, shadowMapSize.y);
	for (int x = -1; x <= 1; ++x)
	{
		for (int y = -1; y <= 1; ++y)
		{
			vec2 uv = vec2(projCoords.x, projCoords.y) + vec2((float)x, (float)y) * texelSize;
			float pcfDepth = toglm(texture2D(shadowMap, uv.x, uv.y)).r;
...
void lighting_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	...
	float shadow = ShadowCalculation(FragPosLightSpace, normal, FragPos, u->lightPos, u->shadowMap);
