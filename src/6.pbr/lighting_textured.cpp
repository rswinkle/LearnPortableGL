...
int main()
{
	...
	// TexCoords, WorldPos, Normal, Tangent. No dFdx/dFdy in PGL; TBN comes from the VS.
	GLenum smooth[] = { PGL_SMOOTH2, PGL_SMOOTH3, PGL_SMOOTH3, PGL_SMOOTH3 };
	GLuint shader = pglCreateProgram(pbr_vs, pbr_fs, 11, smooth, GL_FALSE);
	glUseProgram(shader);
	...
	// Original shader loops 4 lights but only fills one; the rest stay zero.
	for (int i = 0; i < 4; i++) {
		uniforms.lightPositions[i] = vec3(0.0f);
		uniforms.lightColors[i] = vec3(0.0f);
	}
	uniforms.lightPositions[0] = vec3(0.0f, 0.0f, 10.0f);
	uniforms.lightColors[0] = vec3(150.0f, 150.0f, 150.0f);
	...
	while (true)
	{
		...
		{
			mat4 model = mat4(1.0f);
			model = translate(model, uniforms.lightPositions[0]);
...
void pbr_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	*(vec3*)&vs_output[8] = u->normalMatrix * aTangent;
