...
void basic2_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	builtins->gl_FragColor = make_vec4(1.0f, 1.0f, 0.0f, 1.0f);
}
...
int main()
{
	...
	window = SDL_CreateWindow("hello_triangle_exercise3", 100, 100, scr_width, scr_height, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
	if (!window) {
	...
	// ------------------------
	unsigned int shaderProgramOrange = pglCreateProgram(basic_vs, basic1_fs, 0, NULL, GL_FALSE);
	unsigned int shaderProgramYellow = pglCreateProgram(basic_vs, basic2_fs, 0, NULL, GL_FALSE);
