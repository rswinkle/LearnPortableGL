...
void texture_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	...
	builtins->gl_FragColor = mix_vec4s(texture2D(u->tex1, fs_input[3], fs_input[4]), texture2D(u->tex2, fs_input[3], fs_input[4]), 0.2);
}
...
int main()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // note that we set the container wrapping to GL_CLAMP_TO_EDGE
...
void setup_context()
{
	...
	window = SDL_CreateWindow("shaders_interpolation", 100, 100, scr_width, scr_height, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
	if (!window) {
