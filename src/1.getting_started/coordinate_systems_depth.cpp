...
int main()
{
	...
	while (true)
	{
		...
		uniforms.model = glm::rotate(model, SDL_GetTicks()/1000.0f, glm::vec3(0.5f, 1.0f, 0.0f));
...
void setup_context()
{
	...
	window = SDL_CreateWindow("coordinate_systems_depth", 100, 100, scr_width, scr_height, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
	if (!window) {
