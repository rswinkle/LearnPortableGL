...
int main()
{
	...
	while (true)
	{
		...
		uniforms.projection = glm::perspective(glm::radians(fov), (float)scr_width / (float)scr_height, 0.1f, 100.0f);

		// camera/view transformation
		uniforms.view = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);
		...
		for (unsigned int i = 0; i < 10; i++)
		{
			...
			glm::mat4 model = glm::mat4(1.0f); //initialize to identity first
...
void setup_context()
{
	...
	window = SDL_CreateWindow("camera_mouse_zoom", 100, 100, scr_width, scr_height, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
	if (!window) {
